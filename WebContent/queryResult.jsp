		...
		<%
			...
			if (request.getAttribute("results") != null) {
				//获得当前页，总页数，用于显示
					...
					if (pageNow != 1) {
					    out.println("<a  href='QueryDocument?pageNow="+ (pageNow - 1) + "&query="+value+"'>上一页</a>");
					}
				...
				if (!results.isEmpty()) {
					...
					while (resultsIt.hasNext()) {
						...
						out.println("<td><a href='DeleteDocument?id=" + doc.getdId()
								+ "&query="+value+"' onclick='deletDocument()&pageNow="+pageNow+"'>删除</a></td></tr>");
					}
					out.println("</table>");
					...
					for (int i = 1; i <= pageCount; i++) {
					    // 这个href的写法很重要
					    if(i==pageNow) {
					     out.println("<a style='color:red' href='QueryDocument?pageNow=" + i+ "&query="+value+"'>" + i + "</a>");
					    } else {
					...
					if (pageNow != pageCount) {
					    out.println("<a title='下一页' href='QueryDocument?pageNow="+ (pageNow + 1) + "&query="+value+"'>下一页</a>");
					}
					// 显示分页信息
